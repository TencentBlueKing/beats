	...
	for _, path := range p.config.Paths {
		...
		for _, file := range matches {
			...
			if p.config.Symlinks {
				...
				if _, exists := uniqFileID[fileID]; exists {
	...
	if oldState.Finished {
		if oldState.Fileinfo == nil && newState.Fileinfo.Size() < oldState.Offset {
			logp.Err("Old file was truncated. Starting from the beginning: %s, offset: %d, new size: %d,"+
				" old StateOS: %s, new StateOS: %s", newState.Source, oldState.Offset, newState.Fileinfo.Size(), oldState.FileStateOS.String(), newState.FileStateOS.String())
			err := p.startHarvester(newState, 0)
			if err != nil {
			...
			logp.Err("Old file was truncated. Starting from the beginning: %s, offset: %d, old size: %d, new size: %d,"+
				" old StateOS: %s, new StateOS: %s", newState.Source, oldState.Offset, oldState.Fileinfo.Size(), newState.Fileinfo.Size(), oldState.FileStateOS.String(), newState.FileStateOS.String())
			err := p.startHarvester(newState, 0)
			if err != nil {
				logp.Err("Harvester could not be started on truncated file: %s, Err: %s", newState.Source, err)
			}
